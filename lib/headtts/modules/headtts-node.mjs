...
console.log("[HeadTTS] headtts-node.mjs entry loaded at", new Date().toISOString());
    ...
    if ( message.hasOwnProperty("rest") ) {
      ...
      if ( res ) {
        ...
        if ( message.type === "error" ) {
          ...
          // console.log("[HeadTTS] REST synthesize full response:", JSON.stringify(message.data, null, 2));
          // Save the audio as a .wav file for debugging (optional, can be removed in production)
          try {
            const fs = require('fs');
            const path = require('path');
            const outDir = path.join(process.cwd(), 'temporary_files', 'headtts-debug');
            if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
            const outFile = path.join(outDir, `tts_output_${Date.now()}.wav`);
            fs.writeFileSync(outFile, Buffer.from(message.data.audio, 'base64'));
            console.log(`[HeadTTS] Audio file saved to: ${outFile}`);
          } catch (err) {
            // console.error('[HeadTTS] Failed to save audio file:', err);
          }
          // Save the full JSON response for debugging
          try {
            const fs = require('fs');
            const path = require('path');
            const jsonDir = path.join(process.cwd(), 'temporary_files', 'headtts-debug');
            const jsonFile = path.join(jsonDir, `tts_response_${Date.now()}.json`);
            fs.writeFileSync(jsonFile, JSON.stringify(message.data, null, 2));
          //  console.log(`[HeadTTS] JSON response saved to: ${jsonFile}`);
          } catch (err) {
          //  console.error('[HeadTTS] Failed to save JSON response:', err);
          }
          message.data.audio = Buffer.from(message.data.audio).toString('base64');
          // Log the actual response sent to the client (truncated for readability)
          const responsePreview = JSON.stringify(message.data).slice(0, 200) + (JSON.stringify(message.data).length > 200 ? '...' : '');
          console.log("[HeadTTS] Sending REST response:", responsePreview);
